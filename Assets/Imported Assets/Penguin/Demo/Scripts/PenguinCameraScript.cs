...
public class PenguinCameraScript : MonoBehaviour {
	...
	void Update(){
		if (ControlFreak2.CF2Input.GetKey (KeyCode.Mouse1)) {
			cameraAngleY+= ControlFreak2.CF2Input.GetAxis("Mouse X");
			cameraAngleX+= ControlFreak2.CF2Input.GetAxis("Mouse Y");
		}
		CameraRotationX ();
		CameraRotationY ();
		cameraDistance=cameraDistance+.5f*ControlFreak2.CF2Input.GetAxis ("Mouse ScrollWheel");
